...
| S2.3 | `assess-quality` subcommand + docs | S2.2 | S | done | Code + documentation commits complete; `swift test` passed (611 tests, 2 skipped), and both CLI help checks passed on 2026-07-15. Audit 2026-07-15: the shared `--assess-quality` flag appears in `assess-quality --help` and is silently discarded (pipeline forces quality-only) — misleading help text, consider hiding it on this command in a follow-up; the §5.7 skeleton's task-profile forcing lives in `QualityAssessPipeline` rather than the command — accepted, stronger invariant-13 posture. Resolved 2026-07-17: the flag moved out of `SharedOptions` onto `analyze` alone, so `assess-quality` now rejects it as an unknown option. |
...
| S4.2 | Scalar merge/write support | S4.1 | M | done | Existing attributes/elements and all tolerated equal duplicates update in place; absent scalars use attribute form with one namespace declaration on `rdf:RDF`. `swift test` passed (653 tests, 2 skipped) on 2026-07-16. Audit 2026-07-17: the incompatible-prefix fail-closed branch is exercised by S4.6's preview-rejection test — no coverage gap. |
| S4.3 | Fingerprint v2 + snapshot scalars | S4.2 | M | done | Fingerprint v2 excludes scalars only in the owned direct-`rdf:Description` shapes; nested and exact-name foreign-namespace lookalikes remain protected. Snapshots expose all three scalars and fail closed on conflicts. Throw propagation required minimal changes to `OwnedXMPSidecarEngine.swift` and the `XMPMergeValidatorTests` helper beyond the stage's incomplete file list. The plan's named fixture contains unmanaged `aux:rating`, not `xmp:Rating`, so it correctly remains fingerprinted. `swift test` passed (658 tests, 2 skipped) on 2026-07-16. Audit 2026-07-17: the exclusion accepted any `rdf:RDF`-named parent rather than the parser-located element — tightened to an identity match post-audit with a second-island regression test. |
...
| S4.6 | Engine apply path for scalars | S4.5 | M | done | The owned engine previews and applies rating→label→urgency after keyword merge, treats `skip_existing` as a true no-op, includes semantic scalar changes in the write decision, enforces slot/field agreement and Urgency-with-Label, and reports re-read values. Validator arguments and progress flags required minimal `XMPExportPipeline.swift` expansion; the existing restore injection therefore required `XMPExportPipelineTests.swift`, both omitted from the stage file list. Review found and this stage fixed a pre-existing thrown-post-write-validation gap so invalid new targets are removed as well as returned validation failures. `swift test` passed (674 tests, 2 skipped) on 2026-07-16. Audit 2026-07-17: the `wrote_*` progress indicators (pulled early into this stage) lacked a production-path test — added post-audit (progress JSONL for write/skip/disabled); the orphaned `XMPKeywordMerger.preview(plan:snapshot:)` was removed. |
| S4.7 | Quality-grading configuration + write-xmp flags | S3.3, S4.5 | M | done | `swift test` passed (679 tests, 2 skipped) on 2026-07-16; focused configuration/grading/invocation tests and `swift run aisidecar write-xmp --help` also passed. The standard defaults→file→environment→CLI chain now resolves a default-off grading block, validates dormant policies, keeps structured tier maps config-file-only, and preserves legacy report decoding when `quality_grading` is absent. Capture One 16.8.4 evidence supplies reject/Red→1 and excellent/Green→2 urgency defaults. Scope notes per §0 rule 7: `SharedOptions.swift` gained the required ArgumentParser bridge, and `QualityGrading.swift` gained the S4.0-derived defaults plus reusable raw-tier map coding helpers. Two independent post-implementation reviews found no defects. Audit 2026-07-17: the `--quality-min-confidence` enum bridge could silently drop the flag on a future confidence-tier divergence — replaced with an exhaustive switch so divergence becomes a compile error. |
| S4.8 | Planner hookup: grade → plan → write → stamp | S2.4, S4.6, S4.7 | L | done | `swift test` passed (702 tests, 2 skipped) on 2026-07-16. The plan's `Metadata/XMPChangePlanner.swift` path is stale; the planner and plan types share `Metadata/XMPChangePlan.swift`. Scope note per §0 rule 7: `OwnedXMPSidecarEngine.swift` required a fresh-snapshot scalar precondition (including Label/Urgency coupling) so a post-plan app edit cannot be clobbered; the pipeline deliberately does not restore an older backup over that detected edit. The 15-row conflict matrix, final temp-file XMP values, quality-only input, encoded dry-run rows, newest/tied stamp handling, keyword safety, dual contributor stamps, actual scalar ownership, forward-compatible stamp fields, and default-off identity are covered. Two independent final reviews found no remaining defects. Audit 2026-07-17: skip-restore matching moved from a message prefix to the typed `XMPScalarWritePreconditionFailure`; the conflict matrix gained the refresh×absent-with-stamp row (AC-IQ-E6's literal sub-case, now 16 rows); the four new test files (`OwnedXMPScalarPreconditionTests`, `RawSidecarExportStampTests`, `XMPQualityConflictMatrixTests`, `XMPQualityExportIntegrationTests`) are recorded here per §0 rule 7; default-off byte-identity is proven by in-test equivalence rather than a stored golden — accepted. Suite green at 704 tests (2 skipped) after the audit fixes on 2026-07-17. |
| S4.9 | Phase-4 documentation pass | S4.8 | S | done | The Phase 2 F12 addendum, architecture map, CLI implementation notes, and offline verification guide now document managed scalars, grading flags/configuration, 1.1 artifacts, guarded conflicts/stamps, and the Capture One 16.8.4 child-element/Urgency evidence without asserting unverified app read-back. `swift test` passed (702 tests, 2 skipped) on 2026-07-16; both CLI help checks passed; `Scripts/format.sh` completed; three independent read-only documentation audits were addressed. |
| S4.10 | Lightroom pick-flag channel | S4.8 | M | done | Maintainer-directed extension beyond doc 12's scope. Lightroom Classic 15.4.1 samples (`agent_docs/XMP_Samples/Lightroom_Flags/`, GPS stripped; `release-evidence/lr-pick-flag-mapping.md`) show the pick flag as the coupled `xmpDM:pick`/`xmpDM:good` attribute pair: picked 1/true, rejected -1/false, unflagged pick="0" with no good. Both joined the managed scalars end to end (reader/merger/snapshot/fingerprint/validator/preconditions/stamps/reports); the engine refuses split pairs, never writes unflagged, and the planner drives conflict policy from pick with good following its action (unflagged counts as an existing user decision under preserve). New default-on `write_flag` channel with config-file-only `flag_map` (reject→rejected, excellent→picked), `xmp_quality_write_flag`/`AISIDECAR_XMP_QUALITY_WRITE_FLAG`, and paired `--write-flag` CLI flags. Plan/report ids bumped to 1.2 and the fingerprint algorithm to 3.0 with their pinned tests updated deliberately. `swift test` passed (710 tests, 2 skipped) and `write-xmp --help` renders the paired flags on 2026-07-17. Follow-up 2026-07-17 (maintainer decision): the rating channel default flipped to off — keywords, labels/urgency, and pick flags are the default channels and `xmp:Rating` is opt-in; the feature ships as experimental in `0.2.0-beta.1` pending S5.2 read-back evidence. |
...
| S5.4 | Quality scan mode (`combined` \| `sequential`) | S1.2, S2.2, S2.4 | L | done | Maintainer-directed extension beyond doc 12's scope, motivated by sample-set evidence that the combined v1.6.0 call shifts tagging output vs v1.5.0 (~55% keyword overlap on the 18-image reference set, secondary-subject thinning) and rates isolation/masking artifacts as subject-quality problems. `quality_scan_mode` resolves through the standard chain (`--quality-scan-mode` on `analyze`/`write-xmp`/`normalize`, `AISIDECAR_QUALITY_SCAN_MODE`, config key, default `combined`). `sequential` makes `AnalyzePipeline` self-orchestrate tagging (v1.5.0 → `.ai.json`) then quality-only (v1.0.0 → `.quality.ai.json`): per-pass `--existing` resume, whole-run cache lifecycle (clear-on-start pass 1 / clear-after-success pass 2), pass-2 skip on interruption, per-pass batch artifacts. Provenance records the key only when sequential, preserving default-run byte identity (invariant 22); `ResolvedRunConfiguration` gained a hand-written `encode(to:)` for this. Inline analyze-and-write/normalize pair quality records with tagging primaries via the now-internal `groupedSidecarInputs`, and quality-kind plans joined the `writeAIJSON: false` preexisting-sidecar protection. GUI: Step 3 Advanced flags "QUALITY SCAN" control (Normal/High quality, gated on Step 2 assessment), Settings default row writing `quality_scan_mode`, two-pass-aware progress totals, and source-grouped Step 5 outcomes. `swift test` passed (795 tests, 2 skipped, 14 new) on 2026-07-19. Validated same day on the 18-image reference set: sequential tagging bit-identical to the tagging-only baseline across all 26 model runs; dedicated quality pass dropped the masking-artifact false positives, rated focus on 10/18 whole-image (vs 6), confidence high on 26/26, at ~1.8× model wall clock (archived as `Sample_Data_Versioned/0.2.0-beta1/260719_run3` and user-generated `260719_run4`). |
