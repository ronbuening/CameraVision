...
| S4.5 | Change-plan/report type extensions + id bumps | S4.4 | M | done | Added optional scalar plan/request/preview/result/progress/report surfaces, legacy-plan decoding, mock action projection, and 1.1 schema ids. Urgency is intentionally symmetric despite doc 12 §5.6.3 naming only rating/label in one sentence; FR-IQ-040/042/044, AC-IQ-E7, and the Capture One evidence require it. Production `wrote_*` progress values remain nil until S4.8, where the first production scalar-bearing plans and the listed pipeline hookup land; expanding into the pipeline here would violate this stage's file boundary. `swift test` passed (665 tests, 2 skipped) on 2026-07-16. |
| S4.6 | Engine apply path for scalars | S4.5 | M | pending | |
