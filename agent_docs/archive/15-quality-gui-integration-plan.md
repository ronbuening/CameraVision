...
| G6 | Review-step quality surfacing | G3 | M | done | Implemented 2026-07-17. Review re-resolves current contributor inputs through Core and calls `QualityAssessmentExtractor` off the main actor, then shows read-only per-role levels, confidence, strengths/concerns, non-fatal issue diagnostics, Core tier chips, and per-asset summaries. The change-plan sheet projects every retained scalar action/tier/explanation, including verbatim ungraded reasons; completed reports project retained plan plus `XMPWriteResult` before/after values and written/skipped counts. The app contains no assessment parsing, tier derivation, conflict resolution, or quality-keyword construction. Combined/quality-sibling, tolerant-issue, written/`skip_existing`, ungraded, legacy-1.1, and no-quality identity mappings are covered. Focused 32-test gate and full `swift test` passed (772 tests, 2 skipped); source audit, `Scripts/format.sh`, and `git diff --check` passed. |
| G7 | Documentation pass | G1–G6 | S | done | Implemented 2026-07-17. README now documents the experimental Step 2 assessment toggle, Step 3/apply grading controls, read-only Review/plan/report output, and Settings channel/confidence defaults. CLI implementation notes now describe the wired GUI seams, and the architecture map records the quality responsibilities of Run, Review, Export, and Settings. Final `swift test` passed (772 tests, 2 skipped), `swift build --product CupricAspect` and `git diff --check` passed. Advisory `Scripts/format.sh --lint` continues to flag the pre-existing line-break issue at `Tests/AISidecarCoreTests/ModelRuntimeTests.swift:789`; this docs-only stage leaves that unrelated baseline line unchanged. |

Audit 2026-07-18 (post-G7): every stage was re-verified against source and the acceptance criteria. Three corrections landed: the apply-session grading card gained the shared `EXPERIMENTAL` chip (AC-G6 — that surface had no marked gate upstream), `Step3OptionsViewTests` gained `testQualityBoolControlsMapToDistinctOverrideFields` (G3's "each control changes exactly its field" was previously provable with swapped Bool bindings), and `AnalysisRunTests` gained `testAssessQualityOffOverridesEffectiveConfigDefaultOn` (the off direction of G1 Do-2). The audit environment had no macOS toolchain, so these additions are statically verified only — run `swift test` before relying on the counts above.

Audit follow-up 2026-07-18 (maintainer-approved, including the two minimal Core seams — a doc-14-style exception to rule 9 recorded here): (1) `RawJSONSidecarInputResolver.resolveCurrentSidecarPair` is now `public`, and `ReviewModel` quality extraction re-reads the session's stored sidecar references through it instead of a full identity-verified re-resolution — Review now shows exactly what QN6 apply-time grading consumes, with no per-adopt source re-hashing; extraction results are keyed by `sourceAssetID` instead of source-path strings. (2) `XMPChangePlan.ungradedReasonPrefix`/`ungradedReasonExplanation` are the shared Core surface for the `ungraded reason=` explanation; Core and both app consumers use them. (3) `ExportModel` freezes the entire `ResolvedApplySessionConfiguration` at plan time and `confirmWrite()` replays it with only `dryRun` flipped, so config-file edits between plan and confirm cannot alter the committed write. (4) Tier badges in the change-plan sheet and export report use the shared `QualityTierPalette` (a `reject` tier no longer renders green). (5) The `qualityGrading:` parameters of `NormalizationModel.run`/`buildConfiguration`, `ReviewModel.buildSession`/`buildConfiguration`, and `ExportModel.plan`/`applyConfiguration` lost their silent empty defaults — every caller states its overrides. (6) D-G3 stands (maintainer decision): the disabled Step 3 group and README now state that an off assessment also pauses a config-enabled grading default; apply-flow seeding stays once-per-session by decision, documented at `resetApplyQualityDefaults()`. Same toolchain caveat as above: statically verified only.
