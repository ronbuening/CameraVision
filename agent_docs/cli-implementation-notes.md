...
| Show stored assessments and grading outcomes | `QualityAssessmentExtractor`, `XMPChangePlan`, and `XMPExportReport.targetReports[].writeResult` | `ReviewModel` re-resolves current raw-sidecar inputs through Core and calls the extractor off the main actor; `Step5ReviewView` renders the returned records/issues read-only. `ChangePlanSheet` displays retained scalar actions, tier, and verbatim explanation rows. `ExportReportView` displays retained plans plus actual `XMPWriteResult` existing/resulting scalar values. The GUI does not use artifact-only `XMPExportProgressRecord.wrote_*` fields. |
