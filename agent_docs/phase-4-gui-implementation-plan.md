...
1. **Studio "Write XMP" view scope.** Design doc 07 §7 specifies an export-from-existing-sidecars flow with its own folder pickers (FROM RAW SIDECARS + SOURCE ROOT) that has no Wizard equivalent. Decide whether it maps onto `ExportModel`/`Step3ApplyView` or is descoped from M9's chrome-only scope. Note: 08 R2-6 flags an `ExportModel`-bypassing dead path (`NormalizationModel.writeNormalizedXMP` — delete or gate); that resolution affects this decision.
