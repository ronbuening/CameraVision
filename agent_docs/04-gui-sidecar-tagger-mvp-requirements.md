...
AC4-039 - With the post-write cleanup box checked, a successful write leaves the `.xmp` outputs and any `.xmp.bak` in place, removes the run's `.ai.json` sidecars and progress-log/report/summary artifacts from the artifact directory, preserves any normalization session JSON (Apply Prior Session still works), and reports the removed count; a write with any failed target removes nothing; the box is unchecked on every new plan.

AC4-040 - Setting a default concurrency in Settings writes `stage_concurrency` to the shared config with hand-added keys preserved, a subsequent CLI resolve reflects it, and a fresh import's Options concurrency control shows that value; an out-of-range value is clamped/validated before writing; leaving it unset uses the resolver default.

AC4-041 - Setting a default XMP treatment in Settings writes `xmp_conflict_policy` to the shared config and a CLI resolve reflects it; a fresh import's Options → Advanced existing-XMP control shows the saved default (Backup & Merge when unset), matching the CLI; the per-run override still changes only that run and does not write config.

AC4-042 - On both the Options step and Settings, the existing-`.ai.json` control names the program's own analysis sidecars explicitly and is visually distinct from the existing-XMP control; the enum, the `--existing` mapping, and the defaults are unchanged.

AC4-043 - Re-running a folder whose sidecars are mostly skipped shows a per-image rate consistent with the time spent per newly-processed image — or "—" only when zero images were processed — for both the default and an overridden model; a fresh all-written run's rate is unchanged.
...
FR4-063 - **Opt-in post-write cleanup (v0.11).** The XMP-write confirmation (change-plan sheet, shared by the write and normalize flows) shall offer an opt-in checkbox, **off by default**, to remove the run's intermediate files after writing. When checked and the write completes with no failed targets, the GUI shall invoke Core `ArtifactCleanup` over the run's artifact directory (output directory when set, else the source folder), honoring the import's recursive setting. Cleanup shall remove only AISidecar-owned artifacts — `.ai.json` raw sidecars and the run's progress-log/report/summary files — and shall never remove source images, `.xmp` outputs, `.xmp.bak` backups, the derivative cache, debug derivatives, or reusable normalization session JSON. It shall be presentation/state orchestration only (invariant 13 — the deletion logic stays in Core), report the removed count to the user, and be non-fatal (a cleanup error after a successful write is a warning, never a rollback of the delivered XMP). A partial or failed write shall skip cleanup entirely. The checkbox's consequence — that removing `.ai.json` drops the FR4-049 `xmp_export` provenance/audit trail and requires re-analysis to review those images again — shall be stated at the control.

FR4-064 - **Settings default for concurrency (v0.12).** Settings shall let the user set a persistent default for stage concurrency (`stage_concurrency`), persisted to the shared config file per FR4-056 (read-modify-write preserving unknown keys) and validated as greater than zero. The Options step's concurrency control shall pre-populate from the resolved default; a change made there remains a one-time override that does not write config (FR4-060 spirit). Leaving the Settings default unset shall use the resolver default (the machine's performance-core count). The control maps onto the Core `stage_concurrency` value (FR4-044) — no invented values.

FR4-065 - **Settings default for XMP treatment (v0.12).** Settings shall let the user set a persistent default for the existing-XMP conflict policy (Core `XMPConflictPolicy`: Fail / Merge / Backup & Merge, FR4-044), persisted per FR4-056 and defaulting to the Core/CLI built-in `backup-and-merge`. The Options → Advanced existing-XMP control (FR4-061) shall inherit this default by seeding from the resolved config, closing the prior gap in which a config value was ignored by the GUI; the per-run override (FR4-061) shall still override it for a single run without writing config. The GUI default shall equal the Core/CLI built-in so the two never diverge (FR4-056 spirit), and the cross-field rule that `backup-and-merge` requires `backup_sidecars = true` shall be honored.

FR4-066 - **Disambiguated sidecar labeling (v0.12).** The existing-`.ai.json` policy control (Core `ExistingPolicy`) shall be labeled, on both the Options step and Settings, so it unambiguously names the program's own analysis sidecars (`.ai.json`) rather than reading as XMP handling; the existing-XMP control (FR4-061) remains the only XMP-facing control. This is a labeling/caption change only — the enum, the `--existing` mapping, and the defaults are unchanged (FR4-044).

FR4-067 - **Accurate seconds-per-image under skips (v0.12).** The Working step's per-image rate (design resolution 7, seconds-per-image) shall be computed against the images the run actually processed, so a re-run that skips previously-analyzed images (`--existing skip`) reports a correct rate — or a clear "—" when nothing was processed — rather than a figure inflated by dividing whole-run elapsed time by only the newly-written count. The rate is independent of the selected vision model.
