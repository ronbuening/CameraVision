...
5. **C2 — enum divergence is a runtime crash, not a compile error: open (recommendation recorded).** The `CandidateSkipReasonBridgeTests` parity suite is the guard and must stay in CI. The compile-time fix, if wanted as a follow-up item: merge the two 27-case enums into one shared type with a `public typealias` preserving the old name — both encode as raw-value strings, so every document byte is unchanged, and the bridge plus its `preconditionFailure` disappear entirely (a one-sided addition becomes unrepresentable). The existing parity/golden tests are the safety net for that merge.
