...
Follow-up dispositions for the maintainer and the Tranche D executor:

1. **C1 — symlink semantics changed at former `fileExists` sites (open, maintainer decision).** The deliberate `URLResourceValues` consolidation makes `isRegularFile`/`isDirectory` false for symlinks: `ApplySessionPipeline.resolveSourceURL` no longer resolves symlinked source images (falls through to nil with no diagnostic), and `XMPExportPipeline.writesBatchArtifacts`/input-scope classification treat a symlinked directory input as a non-directory. Helper-level tests pin the choice, but no pipeline-level test covers a symlinked source tree. If symlinked photo trees must keep working, follow the symlink before classification (or handle `.isSymbolicLink` explicitly) and add pipeline-level pins; otherwise document the restriction.
2. **D1 anchors — the run config domain spans two files after C7.** The run domain's builder/validation is `struct ConfigurationBuilder` in `RunConfigurationBuilder.swift` (type name does not match the file), separate from `RunConfiguration+Resolve.swift`. Also, `NormalizationConfiguration+Resolve.swift` reached 299 lines after C18 — the next normalization key breaches the C7 ~300-line cap; split a normalization builder file (mirroring the run domain) at that point rather than improvising. A new optional normalization key must touch all six sites (overrides init, env builder, both `apply()` merges, `withoutConfigPath()`, and the manual `init(from:)`/`encode(to:)` using `encodeIfPresent`); the QN3/QN6 artifact-hash fixtures are the only byte-level tripwire for a missed elision.
3. **D4 anchors — C15 moved its targets.** After the recorded C15-before-D4 deviation, D4's file list is stale: the Settings model-discovery section lives in `SettingsModelSection.swift`, tuning-knob controls in `AdvancedOptionsCard.swift`, the model picker in `RunModelPickerCard.swift`, and shared option labels in `Support/WizardOptionLabels.swift`; `PreflightState` stayed in `AnalysisRunModel.swift`. Preserve the split boundaries; while editing the Step-3 cards, dedupe the tripled private `sectionLabel` helper C15 left behind.
4. **D4 — shared `VisionTagsModel.fail()` clears cross-surface state.** An unparseable endpoint entered on one surface clears the shared discovery state and hides runtime guidance (correct while Ollama-only). Per-backend descriptors must scope failure state per backend so one backend's invalid endpoint cannot stomp another's surfaced state.
5. **C2 — enum divergence is now a runtime crash, not a compile error.** A one-sided skip-reason case addition compiles clean and hits the bridge's `preconditionFailure` in production; the `CandidateSkipReasonBridgeTests` parity suite is the only guard — it must stay in CI.
6. **C3 — Phase 2 keyword-merge byte-parity is unit-pinned, not golden-pinned.** Both Phase 2 byte-goldens use empty extractions, and Phase 2's plan-JSON keyword ordering rests on `mergeKeywords`' default `.stable` arguments. Add one keyword-bearing Phase 2 byte-golden before any assembler cleanup that touches merge ordering.
7. **C12 — sentinel tests cannot catch a defaulted new field.** A future `ResolvedXMPExportConfiguration` field added with a memberwise default compiles through all three translators and passes every sentinel test while silently unmapped. Add new fields without memberwise defaults (forcing translator compile errors), or add a field-count assertion to the translator tests.
