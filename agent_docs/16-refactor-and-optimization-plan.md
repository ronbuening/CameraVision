...
- B1. **Stamp the resolved backend into provenance** (audit item 3). Today `run_configuration.model_backend` records the *request*, so an `auto` run encodes `"auto"` and only `model_runs[].runtime` names what ran. Have `VisionModelRunnerFactory` return the resolved backend id alongside the runner and write it into the configuration the pipelines receive, so no artifact ever contains `"auto"`. Files: `ModelRuntime/VisionModelRunnerFactory.swift`, the four CLI analysis call sites, `Features/Run/AnalysisRunModel.swift`. **Acceptance:** `auto` resolving Ollama encodes nothing (default elision) and is byte-identical to a pinned Ollama run; `auto` resolving Apple encodes `"apple"`; a new golden pins the Apple-via-auto shape; the three existing goldens are untouched; a sabotage check confirms an `"auto"` value can no longer reach a sidecar.
