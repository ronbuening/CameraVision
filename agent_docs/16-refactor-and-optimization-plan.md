...
| D8 | deferred | 2026-07-22 | Deferred by maintainer direction. Trigger only when Apple publishes a supported vision-capable FoundationModels image-input API and suitable test hardware is available; then create future scope for live inference, schema-guided generation/repair evidence, capability probing, and any separate benchmark extension. The Tranche D audit adds two items to that future scope: stamp the resolved backend into provenance so `auto` runs never encode `"auto"` (audit item 3), and settle Apple's `supportedTuningKnobs` and the DD-6 CLI ignored-knob notice against the real API (audit items 2 and 6). |
