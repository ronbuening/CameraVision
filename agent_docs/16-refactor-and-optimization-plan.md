...
**Trigger (all three required).** A public, supported FoundationModels API that accepts image input for this workload; a test machine running the OS that ships it; and a maintainer decision to spend the tranche. Nothing below starts before all three hold — every item is unverifiable without a backend that can actually be selected.

**Scope A — the adapter itself.** Production edits confined to `Sources/AISidecarCore/ModelRuntime/AppleFoundationModels/` plus its tests and evidence, per the AC16-5 walkthrough in `agent_docs/cli-implementation-notes.md`:

- A1. Replace `AppleFoundationModelsDescriptor.availability` with a guarded probe requiring both the OS/API and a vision-capable system model; text-only systems stay unavailable with today's reason string.
- A2. Implement `AppleFoundationModelsRunner.prepare` using the already-reserved provenance (`apple-foundation-models`, `system-language-model`, macOS product version, `system:<os build>`) — these are invariant-7 strings, already pinned by `phase1-apple-backend-normalized.json`; never rename them.
- A3. Implement `analyze` against the image-input API: honor cancellation and the shared `ModelRunOptions`, validate every response with the existing `JSONSchemaValidator` against the bundled authoritative schemas (DD-7), and keep any guided-generation or repair bridge private to the adapter.
- A4. Map task profiles the real model cannot satisfy to the already-reserved `E_MODEL_BACKEND_CAPABILITY` (its first production use; the code and its taxonomy test already exist).
- A5. Offline adapter tests plus one explicitly availability-gated live smoke. Invariant 12 still binds: the default suite must stay offline and pass on macOS 15.

**Scope B — follow-ups carried from the Tranche D audit (outside the adapter).** These are refinements, not prerequisites for execution; AC16-5 stays true because ordinary analyze/quality/combined runs light up from Scope A alone. Land each with Scope A, not before.

- B1. **Pin the Apple-via-auto golden** (audit item 3, mostly landed early). The stamping mechanism shipped ahead of D8 by maintainer direction (`53bbec0`): the factory returns the resolved backend id inside the configuration the pipelines receive, `auto` resolving Ollama is test-pinned byte-identical to a pinned Ollama run, `auto` resolving a fake Apple stamps `"apple"`, and the sabotage check was performed. What remains here: a recorded-run golden pinning the Apple-via-auto sidecar shape end to end, once a real second backend can actually be resolved; the three existing goldens stay untouched.
- B2. **Settle Apple's `supportedTuningKnobs`** (audit item 6). The descriptor declares an empty set today, so pinning Apple hides the context-window, timeout, and retry controls in Settings and Step 3. Declare exactly what the real API honors — `timeout` and `retryLimit` are the transport-agnostic candidates. `ModelRunOptions` stays one struct and provenance still records every resolved knob (DD-6); only control visibility changes. **Acceptance:** with Apple selected, the visible controls match the declared set; Ollama's six-knob surface is unchanged; `BackendAwareGUITests` extended.
- B3. **Implement the DD-6 CLI ignored-knob notice** (audit item 2). DD-6's GUI half shipped; the CLI half did not, because no backend ignored anything. Once B2 declares a narrower set, an explicitly passed flag for an unsupported knob must be accepted and logged once per run as a notice — never an error, never a change to stdout artifacts or exit status. Files: `AISidecarCLI/SharedOptions.swift` (which flags were explicit), the four analysis command shapes. **Acceptance:** pinned Ollama logs nothing; pinned Apple with an unsupported flag logs exactly one notice and still runs; dry-run output bytes unchanged.
- B4. **Benchmark support stays separate.** DD-8's guard (`benchmark` rejects every backend except Ollama) remains until someone plans the axis rework — its matrix is Ollama-shaped (`keep_alive`, `ollama --version` metadata). Do not fold it into D8 silently.

**Constraints that still bind.** Invariant 7 on every raw string above; invariant 11 (macOS 15 minimum — availability-gate, never raise the floor); invariant 12 (default suite offline and availability-independent); the additive, default-elided encoding rule for any new configuration key.
...
| D8 | deferred | 2026-07-22 | Deferred by maintainer direction. Trigger only when Apple publishes a supported vision-capable FoundationModels image-input API and suitable test hardware is available; then create future scope for live inference, schema-guided generation/repair evidence, capability probing, and any separate benchmark extension. The D8 item now carries an executable scope split: Scope A (A1–A5) is the adapter work inside `ModelRuntime/AppleFoundationModels/`; Scope B (B1–B4) carries the Tranche D audit follow-ups that live outside it — the Apple-via-auto provenance golden (the resolved-backend stamping itself landed early by maintainer direction, `53bbec0` — see audit item 3), Apple's tuning-knob set, the DD-6 CLI ignored-knob notice, and the still-separate benchmark extension. |
...
2. **DD-6 CLI notice not implemented.** The descriptor half of DD-6 shipped (`supportedTuningKnobs` filters the GUI controls), but "CLI flags for inapplicable knobs are accepted and ignored with a logged notice" did not. It is currently unreachable: Ollama declares all six knobs and Apple can never be selected, so no notice could ever fire. Implement it with the live Apple adapter (D8 item B3), where the first genuinely-ignored knob appears.
3. **`auto` is recorded verbatim in provenance — scheduled for D8.** `run_configuration.model_backend` encodes the *requested* value, so an `auto` run pins `"auto"` rather than the backend that actually ran; only `model_runs[].runtime` identifies it. Harmless today (auto always resolves Ollama, and `runtime` disambiguates) but ambiguous the day two backends are live, and `"auto"` is a permanently load-bearing raw value once it ships in sidecars (invariant 7). **Recommended resolution, to land with the live adapter and not before:** have the factory stamp the resolved backend id back into the configuration the pipelines receive, so provenance always names a concrete backend and never `"auto"`. That is strictly better on bytes as well — an `auto` run that resolves Ollama then encodes nothing at all (default elision) and is byte-identical to a pinned Ollama run, while an Apple resolution encodes `"apple"`. Doing it now would be an unverifiable one-sided change: no second backend can be resolved, so the interesting case cannot be exercised end to end. Gate it on D8 (item B1) and pin it with a golden refresh.

   **Resolved early (2026-07-22, maintainer direction, `53bbec0`).** The maintainer weighed the deferral against the fact that every real `auto` run was accumulating permanently load-bearing `"auto"` strings in the wild, and directed that the reachable half land now. `VisionModelRunnerFactory.make` returns a `Selection` — the runner plus a configuration whose `model_backend` names the concrete backend the factory chose — and the four CLI analysis call sites and the GUI run path hand that configuration to the pipelines. `auto` resolving Ollama now encodes nothing and is proven byte-identical to a pinned Ollama run; fake-descriptor tests also pin that `auto` resolving Apple stamps `"apple"`. The prescribed sabotage was performed: with stamping removed, seven factory tests failed, including the byte-parity guarantee. The same commit closed a latent gap the audit had not flagged: `auto` planning runs (`dry_run`) previously carried the registry's *first* descriptor's runner — the inert Apple stub in production — and now prefer the default backend's descriptor, still without any availability probe. D8 item B1 retains only the golden that pins the Apple-via-auto sidecar shape, which needs a resolvable second backend.

Fixed after the audit (`1fbb4e7`):

4. **Availability probing was expensive and duplicated.** `OllamaBackendDescriptor.availability` called `discoverModels` → `listInstalledVisionTags`, issuing `/api/tags` plus one `/api/show` per installed model to answer a yes/no reachability question that `prepare` then re-answered. It now issues a single `/api/version` via `OllamaVisionRunner.probeReachability`; model enumeration stays where it belongs, in `discoverModels`. Availability semantics are unchanged — both shapes meant "the endpoint responds", since a reachable Ollama with zero vision models was already `.available` and still fails later in `prepare` with `E_MODEL_TAG_NOT_FOUND`. Both the probe and discovery now pass the resolved `model_timeout_seconds` instead of using the built-in default. Tests pin the single `/api/version` request and the timeout pass-through.
5. **Settings annotations now come from the descriptor probe.** `SettingsModel.loadBackendData` probed every backend except the selected one and then overwrote the selected entry from the vision-tags result, so the picker could annotate a backend differently from the answer the factory would act on. It now probes every descriptor — cheap after item 4 — and discovery state answers only which models the selected backend offers. Pinned-but-unavailable backends short-circuit discovery the way `auto` already did. Test pinned: a reachable backend whose discovery fails stays "available" in the picker while the model list reports the discovery error.

   **CI follow-up (2026-07-22, `c2b6634`).** The tranche's first CI run (PR #60) caught a test this change left environment-dependent: `VisionTagsModelTests.testSettingsProjectsSharedStateAndApplyingEndpointForcesRefresh` injected a fake vision-tag loader but kept the live registry, so the new availability gate probed the real Ollama endpoint — passing wherever Ollama runs (the dev machine) and timing out on CI where the refused probe short-circuited discovery before the fake loader could be called (invariant 12). The test now injects an always-available fake descriptor, the pattern `BackendAwareGUITests` already uses; an audit of the probe-gated paths (`loadVisionTagsIfNeeded`, `applyEndpoint`, `refreshVisionTags`, `setModelBackend`) found no other test with the exposure.

Deferred deliberately:

6. **Pinning `apple` hides Settings controls.** Apple declares no tuning knobs, so pinning it hides the context-window, timeout, and retry controls, and `usesEndpoint` hides the Ollama endpoint field. This is DD-6/DD-9 behaving as designed and it strands nobody — the backend picker stays visible, so switching back restores every control. Which knobs Apple actually honors cannot be known before its image-input API exists, so the knob set is a D8 decision (item B2, with the DD-6 CLI notice as B3); the transport-agnostic candidates (`timeout`, `retryLimit`) should be re-examined then rather than guessed now.
