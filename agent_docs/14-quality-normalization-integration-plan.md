...
Version: 1.0
Date: 2026-07-17
Status: implemented (QN1–QN8 complete)
...
| QN6 | Grading at apply-session time | QN3, QN5 | M | done | `apply-session` now consumes the shared grading option group and resolves the existing config/environment keys through an additive apply-session grading block. When enabled, it re-resolves current contributor documents, reads current XMP, and rebuilds grading from those inputs; stored preview grading is never authoritative, and a removed assessment reports `no_records`. Reality note: the normal raw-sidecar resolver also re-verifies source identity, so a document-only current-pair resolver was added after apply-session's existing source verification to preserve `--allow-stale`; matched contributors are still checked against the session asset identity, surviving tag/quality siblings are remapped in memory with nested relative paths preserved, and the session file is not mutated. A pre-QN6 fixture pins all six default-off artifact hashes and asserts no new sidecar/XMP planning reads. End-to-end tests cover current-assessment mutation, current-XMP conflicts, lost assessments, legacy sessions off/on, wrong-source sibling rejection, stale-source opt-in, and a surviving nested quality sibling. `swift test` passed (745 tests, 2 skipped); post-format focused tests passed (83 tests); apply-session help/declaration checks and `git diff --check` passed; adversarial review found no remaining issues; `Scripts/format.sh` completed on 2026-07-17. |
| QN7 | GUI enablement seam (Core-side only) | QN3, QN4, QN5 | S | done | The app's configuration and result call sites are documented with their exact Core seams; no additive override gap or GUI/Core source change was needed. Reality note: contrary to D-QN5/stage shorthand, only `AnalysisOptions` currently constructs an override struct; `NormalizationModel` and `ReviewModel` mutate resolved normalization defaults directly, while `ExportModel` mutates resolved apply-session defaults. The later GUI pass must route those helper methods through `resolveNormalization`/`resolveApplySession` to honor config and environment precedence, keep authoritative review grading in `ExportModel`, and pass identical grading overrides to dry-run and write. The docs also distinguish live `XMPExportReport` before/after scalar values from artifact-only `XMPExportProgressRecord.wrote_*` fields. `swift test` passed (745 tests, 2 skipped); independent seam audit and `git diff --check` passed; `Scripts/format.sh` completed on 2026-07-17. |
| QN8 | Documentation pass | QN1–QN7 | S | done | Public and maintainer documentation now leads with the one-command `normalize --assess-quality --quality-grading` flow, states that quality keywords bypass vocabulary/consensus and remain additive, and records current-state apply-session grading. The shared `QualityGradingOptions` help surface, all three config consumers, normalized pipeline entry points, offline graded-normalize smoke, and GUI-deferred boundary are documented. Stale write-xmp-only and target-path-only claims were removed; the config template now names normalize/apply-session without adding keys. All three grading help surfaces were manually verified. `swift test` passed (745 tests, 2 skipped); independent documentation audit and `git diff --check` passed; `Scripts/format.sh` completed on 2026-07-17. |

## 2. Design overview

### Problem

Before this plan, quality assessment/grading (phase 4 + S4.10) and keyword normalization were separate pipelines. Composing them took three commands, and the grading-only `write-xmp` pass had to disable the flat/hierarchical keyword channels to avoid diluting normalized keywords — which also suppressed the deterministic `AI Quality|<tier>` quality keywords, because they rode the same channel gates. `normalize` knew nothing about quality: no `--assess-quality` in its analyze mode, no grading in its write path.
