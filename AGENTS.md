...
CameraVision is a Swift 6, macOS 15, SwiftPM project for local-first AI-assisted photo metadata. The single CLI executable `aisidecar` scans image folders, renders model inputs, runs a selected local vision backend (Ollama is the usable backend today; the Apple FoundationModels adapter ships deliberately unavailable), writes auditable raw `.ai.json` sidecars (Phase 1), exports accepted keywords to XMP sidecars through a project-owned XMP engine (Phase 2), and performs vocabulary-normalized batch tagging with durable, replayable sessions (Phase 3). The SwiftUI GUI (Phase 4) is the app `CupricAspect` (`Sources/CupricAspectApp`, run via `swift run CupricAspect`): milestones M0–M8a and beta-readiness items B0-1–B0-4 and B0-6 are done. The current full-release line is `0.2.0`, packaged via `Scripts/build-release.sh`; Developer ID signing/notarization is deliberately deferred for this release, so its downloadable DMG remains ad-hoc signed and not notarized. M9–M11 (Studio shell, database mode, scale) are post-release.

**Status:** Phase 1 Milestones 0–9a (including the 2026-07-20 Milestone 9 calibration signoff), Phase 2 Milestones 0–10, the GPS-context milestone, Phase 3 Milestones 0–11, Phase 4 GUI milestones M0–M8a plus B0-1–B0-4/B0-6, and hardening R1–R4 are implemented; R4 received a post-implementation adversarial audit on 2026-07-11 and a second verification audit on 2026-07-14 (corrections and accepted residuals recorded in plan 08 §5 and archived plan 10). Efficiency P2/P3 are complete inside R4-6. The experimental Image Quality Assessment feature (plans 12–15) is implemented end to end for `0.2.0`: model-contract stages S0–S1, the quality-only pipeline S2, extraction/grading S3, XMP managed scalars S4 (including the Lightroom pick-flag channel S4.10), quality-aware normalization QN1–QN8, GUI integration G1–G7, and the S5.4 quality scan mode (`combined` | `sequential` — sequential runs tagging and quality as two passes so tagging output stays byte-identical to a no-assessment run). Manual quality-feature evidence stages S5.1–S5.3 remain open and the feature stays visibly experimental and opt-in. Refactor/optimization plan 16 is implemented through Tranche D7; D8 live Apple inference is explicitly deferred until a public vision-capable API and suitable test hardware exist. Do not reopen completed milestone work without new acceptance criteria.
...
- Resolve model backends through Core's `VisionBackendRegistry`/`VisionModelRunnerFactory`; CLI and GUI surfaces consume backend descriptors rather than branching on a concrete runtime. Backend-specific implementation stays under `Sources/AISidecarCore/ModelRuntime/`.
...
- `agent_docs/16-refactor-and-optimization-plan.md`: implemented refactor/optimization ledger. Tranche A absorbs plan 05's remaining items by reference; Tranches B/C add performance and structure work; Tranche D1–D7 land the vision-backend abstraction (`model_backend` config, runner factory, backend descriptors, backend-aware GUI) with a compile-gated, deliberately-unavailable Apple FoundationModels adapter. D8 remains deferred pending a vision-capable public API and test hardware.
- `agent_docs/05-efficiency-improvement-plan.md`: item-level spec for plan 16's Tranche A (its remaining P/R items). P2/P3 are already complete inside R4-6 and must not be scheduled again.
- `agent_docs/17-gui-improvements-plan.md`: secondary GUI improvements plan (accessibility pass, review filter/search/keyboard triage, inline error component, ETA, drop feedback) — runs after plan 16 Tranches A–C at maintainer priority; does not gate M9.
