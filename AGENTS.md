...
- `Sources/AISidecarCore/Normalization` owns Phase 3 vocabulary loading/indexing, artifact planning, normalization input resolution, candidate observations, vocabulary canonicalization, direct per-asset decisions, session schema records, source-asset records, and early affinity input audit records.
...
- The next planned implementation unit is Phase 3 Milestone 4 metadata-affinity graph, local weighted consensus, and session-context conflict handling from `agent_docs/03-cli-normalized-batch-tagger-requirements.md`.
