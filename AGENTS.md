...
- `Sources/AISidecarCore/Pipeline` owns the full analyze pipeline, the earlier analyze shell pipeline test seam, the diagnostic model-input export pipeline, XMP export pipeline, analyze-and-write adapter, Phase 3 session/dry-run normalize pipeline, and interruption handling.
...
- The next planned implementation unit is Phase 3 Milestone 6 reports, summaries, progress logs, and session-only finalization from `agent_docs/03-cli-normalized-batch-tagger-requirements.md`.
