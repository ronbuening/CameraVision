...
CameraVision is a Swift 6 macOS 15 SwiftPM project for local AI-assisted photo metadata workflows. The current implemented state is Phase 1 Milestone 2: package scaffold, CLI surface, config resolution, structured errors, logging, scanner/source identity, `--dry-scan`, sidecar naming, output tree mirroring, atomic raw JSON sidecar shell writes, JSONL progress logs, batch summaries, interruption handling, and offline tests.
...
- `Sources/AISidecarCore/Sidecars` owns raw `.ai.json` sidecar naming and atomic shell writes.
- `Sources/AISidecarCore/Reporting` owns text/JSON logging, JSONL progress logs, and batch summaries.
- `Sources/AISidecarCore/Pipeline` owns the current Milestone 2 analyze shell pipeline.
...
- Manual Milestone 2 smoke check: `swift run aisidecar analyze <folder> --recursive --output-dir <tmp-output>`.
...
- The next planned unit is Phase 1 Milestone 3: rendering and derivative cache. Start with model input profiles, render recipes, orientation baking, sRGB output, full-resolution render retention, content-addressed cache keys, and focused offline tests.
- Do not jump ahead to subject isolation, model runtime, or XMP writing while implementing Milestone 3.
