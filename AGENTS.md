...
- `agent_docs/16-refactor-and-optimization-plan.md`: **the next scheduled code plan** (after R4, before M9). Tranche A absorbs plan 05's remaining items by reference; Tranches B/C add new performance and structure items from the 2026-07-21 review; Tranche D lands the vision-backend abstraction (`model_backend` config, runner factory, backend descriptors, backend-aware GUI) with a compile-gated, deliberately-unavailable Apple FoundationModels adapter awaiting a vision-capable API.
