...
- `Sources/AISidecarCore/Pipeline` owns the full analyze pipeline, the earlier analyze shell pipeline test seam, the diagnostic model-input export pipeline, XMP export pipeline, analyze-and-write adapter, Phase 3 session/dry-run normalize pipeline, apply-session pipeline, and interruption handling.
...
- The next planned implementation unit is Phase 3 Milestone 8 analyze-and-normalize integration from `agent_docs/03-cli-normalized-batch-tagger-requirements.md`.
